...
class PricerequestController extends BaseController {
    ...
    public function price_request() {
        ...
        require $this->load_template('templates', DIRECTORY_SEPARATOR . 'propeller-price-request.php');
    }
    ...
    public function add($product) {
        ...
        if (!$this->check($product)) {
            ...
            $response->message = __('Product is added in your price request list. Please go to your account menu to send your price request list.', 'propeller-ecommerce');
        }
        else {
            $response->success = false;
            $response->error = true;
            $response->message = __('Product already exists in price request list', 'propeller-ecommerce');
        }
    ...
    public function remove($product) {
        ...
        if ($this->check($product)) {
            ...
            $response->success = true;
            $response->error = false;
            $response->message = __('Product removed from price request list', 'propeller-ecommerce');
        }
        else {
            $response->success = false;
            $response->error = true;
            // $response->message = __('Product doesn\'t exists in price request list', 'propeller-ecommerce');
        }
    ...
    public function send_price_request($data) {
        ...
        require $this->load_template('emails', DIRECTORY_SEPARATOR . 'propeller-price-request-template.php');
        ...
        $response->success = wp_mail($user_email, __('Price request', 'propeller-ecommerce'), $email_content, implode("\r\n", $headers));
        // $response->postprocess->success = mail($args['return_email'], __('Return request', 'propeller-ecommerce'), $email_content, implode("\r\n", $headers));

        $msg = __('Price request sent. We will contact you.', 'propeller-ecommerce');
    ...
    private function check($product) {
        ...
        $found = array_filter($products, function($obj) use ($product) { return $obj->code == $product->code; });
    ...
    private function remove_product($product) {
        ...
        $arr = array_filter($products, function($obj) use ($product) { return $obj->code !== $product->code; });
