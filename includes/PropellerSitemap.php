...
class PropellerSitemap extends BaseController {
    ...
    public function build_sitemap() {
        ...
        if ($this->is_sitemap_valid()) {
            ...
            $return->success = true;
            $return->message = __('Sitemap files are generated 1.', 'propeller-ecommerce');
            $return->reload = true;
        ...
        $propel_settings = $wpdb->get_row("SELECT * FROM " . $table_prefix . PROPELLER_SETTINGS_TABLE);
        $behavior_result = $wpdb->get_row("SELECT * FROM " . $table_prefix . PROPELLER_BEHAVIOR_TABLE);

        $base_catalog_id = $propel_settings->catalog_root;
        ...
        if (isset($languages->languages) && is_array($languages->languages) && count($languages->languages)) {
            foreach ($languages->languages as $locale => $url) {
                ...
                for ($i = 0; $i <= $product_sitemap_files_count; $i++) {
                    $file = ABSPATH . "propel-sitemap-products-$url-" . ($i + 1) . ".xml";
                    ...
                    $file_data->file = $file;
                    $file_data->lastmod = date(DATE_ATOM);
                ...
                for ($i = 0; $i <= $categories_sitemap_files_count; $i++) {
                    $file = ABSPATH . "propel-sitemap-catalog-$url-" . ($i + 1) . ".xml";
                ...
                for ($i = 0; $i <= $brands_sitemap_files_count; $i++) {
                    $file = ABSPATH . "propel-sitemap-brands-$url-" . ($i + 1) . ".xml";
        ...
        $return->success = true;
        $return->message = __('Sitemap files are generated 2.', 'propeller-ecommerce');
        $return->reload = true;
        $return->page = 'propeller';
        $return->tab = 'sitemap';
    ...
    private function build_product_sitemap($page_slug, $language, $avoid_lang_prefix, $use_ids_in_urls, &$sitemap_products, &$product_sitemap_files_count) {
        for ($i = 1; $i <= $this->products_pages; $i++) {
            ...
            foreach ($products->items as $product) {
                if (isset($product->slug) && is_array($product->slug) && count($product->slug)) {
                    $url_chunks = [];
    ...
    private function get_sitemap_files() {
        ...
        $path  = ABSPATH . 'propel-sitemap-*.xml';
		$files = glob( $path );
        ...
        if (!$files_valid)
            ...
            foreach ($files as $sitemap_file) {
                ...
                $file->lastmod = date(DATE_ATOM, filemtime($sitemap_file));
                $propel['sitemap_files'][] = $file;
            }
        }
    }
    ...
    public function get_files() {
        ...
        foreach ($files as $sitemap_file) {
            ...
            $file->url = home_url('/' . basename($sitemap_file));
            $file->lastmod = date('Y-m-d H:i:s', filemtime($sitemap_file));
            $sitemap_files[] = $file;
        }
    ...
    public function has_index() {
        ...
        return file_exists(ABSPATH . 'sitemap.xml');
    }
    ...
    public function get_index() {
        ...
        $file->url = home_url('/sitemap.xml');
        $file->lastmod = date('Y-m-d H:i:s', filemtime($path));
