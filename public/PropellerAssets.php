...
class PropellerAssets {
	...
	private function handle_mode_global_combined() {
		...
		add_action( 'wp_enqueue_scripts', function () {
			...
			foreach ( $this->assets as $type => $list ) {
				foreach ( $list as $handle => $asset ) {
					if ( $this->is_cdn_source( $asset['js'] ) && ( ! isset( $asset['js']['condition'] ) || $asset['js']['condition'] ) ) {
						wp_enqueue_script( $handle, $asset['js']['src'], [], null, true );
					}
					if ( $this->is_cdn_source( $asset['css'] ) && ( ! isset( $asset['js']['condition'] ) || $asset['js']['condition'] ) ) {
	...
	public function std_enqueue_global_assets() {
		...
		wp_set_script_translations( 'propeller-global', 'propeller-ecommerce', PROPELLER_PLUGIN_EXTEND_DIR . DIRECTORY_SEPARATOR . 'languages' . DIRECTORY_SEPARATOR );
	}
	...
	public function std_register_asset( $handle, $asset, $external = false ) {
		if ( isset( $asset['js'] ) ) {
			if ( $this->is_cdn_source( $asset['js'] ) ) {
				...
				$version   = isset( $asset['js']['version'] ) ? $asset['js']['version'] : false;
				wp_register_script( $handle, $asset['js']['src'], $deps, $version, $in_footer, );
			} else {
				$paths     = $this->get_asset_src( 'js', $asset['js']['src'], $external );
				$deps      = isset( $asset['js']['deps'] ) ? $asset['js']['deps'] : [];
				$version   = isset( $asset['js']['version'] ) ? $asset['js']['version'] : ( defined( 'WP_DEBUG' ) && WP_DEBUG ? filemtime( $paths['file_path'] ) : false );
				$in_footer = isset( $asset['js']['in_footer'] ) ? $asset['js']['in_footer'] : true;
	...
	public function get_assets_map() {
		...
		$map = apply_filters( 'propel_frontend_assets', [
			'external' => [
				...
				'propeller-recaptcha' => [
					'js' => [
						'src'       => 'https://www.google.com/recaptcha/api.js?render=' . ( Propeller::use_recaptcha() && defined('PROPELLER_RECAPTCHA_SITEKEY') ? PROPELLER_RECAPTCHA_SITEKEY : '' ) . '&lang=' . strtolower(PROPELLER_LANG),
						'cdn'       => true,
