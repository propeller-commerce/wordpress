...
use Propeller\Includes\Enum\CrossupsellTypes;
    ...
    $cluster_product = $this->product->defaultProduct ? $this->product->defaultProduct : $this->product->products[0];
?>
...
<div class="container-fluid px-0 propeller-product-details <?php echo apply_filters('propel_product_details_classes', ''); ?>">
        <?php echo apply_filters('propel_cluster_gecommerce', $this->product, $cluster_product, $this); ?>
        ...
        <div class="row">
            <div class="col">
                <?php
                    ...
                    $breadcrumb_paths[] = [
	                    $this->buildUrl(PageController::get_slug(PageType::PRODUCT_PAGE), $cluster_product->slug[0]->value, $cluster_product->urlId),
        ...
        <div class="row product-gallery-price-wrapper">
            ...
            <div class="col-12 col-lg-7 gallery-wrapper">
                <?php echo apply_filters('propel_cluster_name_mobile', $this->product); ?>

                <?php echo apply_filters('propel_cluster_product_name_mobile', $cluster_product); ?>
                
                <?php echo apply_filters('propel_cluster_meta_mobile', $this->product, $cluster_product, $this); ?>

                <?php echo apply_filters('propel_cluster_gallery', $cluster_product, $this); ?>

                <?php echo apply_filters('propel_cluster_desc_media', $this->product, $cluster_product, $this); ?>
            </div>
            
            <!-- Product name, pricing, short description -->
            <div class="col-12 col-lg-5">
                <div class="product-price-description-wrapper">
                    <?php echo apply_filters('propel_cluster_name', $this->product); ?>

                    <?php echo apply_filters('propel_cluster_product_name', $cluster_product); ?>

                    <?php echo apply_filters('propel_cluster_meta', $this->product, $cluster_product, $this); ?>
                    <?php if (!(!UserController::is_logged_in() && PROPELLER_WP_SEMICLOSED_PORTAL)) { ?> 
                        <?php echo apply_filters('propel_cluster_price_details', $cluster_product); ?>
                    <?php } ?>
                    <?php echo apply_filters('propel_cluster_options', $this->product, $cluster_product, $this); ?>
                </div>
            </div>
        </div>

        <?php echo apply_filters('propel_cluster_bundles', $cluster_product, $this); ?>
        ...
        <?php echo apply_filters('propel_crossupsells_ajax', $this, CrossupsellTypes::RELATED); ?>
            ...
            <div id="fixed-wrapper" class="d-md-none fixed-wrapper <?php if (sizeof($cluster_product->bulkPrices) > 1) { ?>has-bulk-prices<?php } ?>">
                <?php if (!(!UserController::is_logged_in() && PROPELLER_WP_SEMICLOSED_PORTAL)) { ?>    
                    <?php echo apply_filters('propel_product_bulk_prices', $cluster_product); ?>
                <?php } ?>
                <div class="row align-items-center justify-content-between">
                    
                    <?php echo apply_filters('propel_product_add_to_basket', $cluster_product); ?>

                    <?php /* <div class='col-auto pr-30'>
                        <?php echo apply_filters('propel_product_add_favorite', $cluster_product); ?>
                        */ ?>
                    </div>     
                </div>
            </div> 
