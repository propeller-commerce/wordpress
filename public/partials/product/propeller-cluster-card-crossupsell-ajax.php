...
<div class="card propeller-product-card">
    ...
    <div class="card-footer product-card-footer">
        ...
        <div class="product-price">
            <?php if(!empty($cluster_product->storePrice)) { ?>
                <?php if ($user_prices == false) { ?>
                   <div class="product-current-price has-discount d-md-inline-flex"><span class="price"><?= PropellerHelper::formatPrice($cluster_product->price->gross); ?><div class="product-old-price d-md-inline-flex"><span class="price"><?= PropellerHelper::formatPrice($crossupsell->item->storePrice); ?></span></div> <small><?php _e( 'excl VAT', 'propeller-ecommerce' ); ?></small></span>
                   
                </div>
                <?php } else { ?> 
                    <div class="product-current-price has-discount d-md-inline-flex"><span class="price"><?= PropellerHelper::formatPrice($cluster_product->price->net); ?> <small><?php _e( 'incl. VAT', 'propeller-ecommerce' ); ?></small></span></div>    
                <?php } ?>
                
               
                <?php } else if ($user_prices == false) { ?>
                    <div class="product-current-price"><span class="price"><?= PropellerHelper::formatPrice($cluster_product->price->gross); ?> <small><?php _e( 'excl VAT', 'propeller-ecommerce' ); ?></small></span></div>
                <?php } else { ?>
                
                    <div class="product-current-price"><span class="price"><?= PropellerHelper::formatPrice($cluster_product->price->net); ?><small><?php _e( 'incl. VAT', 'propeller-ecommerce' ); ?></small></span></div>
                <?php } ?>	
        </div>
        ...
        <div class="product-code"><?= $cluster_product->manufacturer; ?> / <?= $cluster_product->sku; ?></div>
        ...
        <div class="product-card-short-description">
            <?= $cluster_product->shortDescription[0]->value;?>                                    
        </div>
    </div>
</div>