...
<div class="col-auto d-none d-sm-flex align-items-center catalog-offset">
    ...
    <div class="dropdown sticky-dropdown-menu">
        <select name="catalog-offset" class="form-control" data-prop_name="<?= $prop_name; ?>" data-prop_value="<?= $prop_value; ?>" data-action="<?= $do_action; ?>">
        <?php foreach ($obj->offset_arr as $o) { ?>
            <option value="<?= $o; ?>" <?php echo (isset($_REQUEST['offset']) && (int) $_REQUEST['offset'] == $o) ? 'selected' : ''; ?>><?= $o; ?></option>
        <?php } ?>
        </select>
    </div>
</div>
<div class="col-auto d-flex align-items-center catalog-sort">
    ...
    <div class="dropdown sticky-dropdown-menu">
        <select name="catalog-sort" class="form-control" data-prop_name="<?= $prop_name; ?>" data-prop_value="<?= $prop_value; ?>" data-action="<?= $do_action; ?>">
        <?php foreach ($obj->sort_arr as $sort_key => $sort_val) { ?>
            <?php foreach ($obj->sort_order as $order_key => $order_val) { ?>
                <option value="<?= $sort_key . ',' . $order_key; ?>" <?php echo $sort == $sort_key . ',' . $order_key ? 'selected' : ''; ?>><?php echo __($sort_val . ' ' . $order_val, 'propeller-ecommerce'); ?></option>
            <?php } ?>
        <?php } ?>
        </select>
    </div>
